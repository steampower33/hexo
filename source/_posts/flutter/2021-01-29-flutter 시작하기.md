...
```
2-1. 먼저 flutter.bat파일을 우클릭, 속성 누른 후 위치의 주소를 복사
2-2. 그 뒤 윈도우키를 누르고, 시스템 환경 변수 편집을 실행
2-3. 환경변수를 누르고, 위 아래 중 아래의 시스템 변수에서 휠로 내림면 변수위치에 Path가 있음
2-4. Path를 클릭 후, 편집, 새로만들기한 후 아까 복사한 주소를 붙여넣기 후 확인
2-5. 또 확인하면 환경변수 설정 완료
```
